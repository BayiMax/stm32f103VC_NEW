...
static
int get_ldnumber (		/* Returns logical drive number (-1:invalid drive) 返回逻辑驱动器号(-1:无效驱动器) */
	const TCHAR** path	/* Pointer to pointer to the path name 指向路径名的指针 */
)
{
	const TCHAR *tp, *tt;
	UINT i;
...
BYTE check_fs (	/* 0:FAT boor sector, 1:Valid boor sector but not FAT, 2:Not a boot sector, 3:Disk error */
	FATFS* fs,	/* File system object 文件系统对象*/
	DWORD sect	/* Sector# (lba) to check if it is an FAT boot record or not ． 扇区# (Iba)检查它是否为FAT引导记录*/
)
{
	...
	if ((LD_DWORD(&fs->win[BS_FilSysType]) & 0xFFFFFF) == 0x544146){		/* Check "FAT" string 检查“FAT”字符串*/
		return 0;
	}
	if ((LD_DWORD(&fs->win[BS_FilSysType32]) & 0xFFFFFF) == 0x544146){	/* Check "FAT" string 检查“FAT”字符串*/
		return 0;
	}
	return 1;
}
...
static
FRESULT find_volume (	/* FR_OK(0): successful, !=0: any error occurred FR OK (O):成功，!=0:任何错误发生*/
	FATFS** rfs,		/* Pointer to pointer to the found file system object 指向找到的文件系统对象的指针*/
	const TCHAR** path,	/* Pointer to pointer to the path name (drive number) 指向路径名(驱动器号)的指针*/
	BYTE wmode			/* !=0: Check write protection for write access =0:检查写访问写保护*/
)
{
	BYTE fmt, *pt;
	int vol;
	...
	*rfs = 0;
	...
	/* Check if the file system object is valid or not 检查文件系统对象是否有效*/
	fs = FatFs[vol];					/* Get pointer to the file system object 获取指向文件系统对象的指针*/
	if (!fs) return FR_NOT_ENABLED;		/* Is the file system object available? 文件系统对象是否可用?*/

	ENTER_FF(fs);						/* Lock the volume */
	*rfs = fs;							/* Return pointer to the file system object 返回指向文件系统对象的指针*/

	if (fs->fs_type) {					/* If the volume has been mounted 如果卷已经挂载*/
	...
	printf("fmt=%d\r\n",fmt);
	if (fmt == 1 || (!fmt && (LD2PT(vol)))) {	/* Not an FAT boot sector or forced partition number ?不是FAT引导扇区或强制分区号*/
...
/*-----------------------------------------------------------------------*/
/* Mount/Unmount a Logical Drive 挂载/卸载逻辑驱动器                     */
/*-----------------------------------------------------------------------*/

FRESULT f_mount (
	FATFS* fs,			/* Pointer to the file system object (NULL:unmount) 指向文件系统ob对象的指针(NULL: unmount)*/
	const TCHAR* path,	/* Logical drive number to be mounted/unmounted 待挂载/卸载的逻辑驱动器号*/
	BYTE opt			/* 0:Do not mount (delayed mount), 1:Mount immediately  0:不挂载(延迟挂载);1:立即挂载*/
)
{
	FATFS *cfs;
	int vol;
...
#if _FS_REENTRANT						/* Create sync object for the new volume 为新卷创建同步对象*/
		if (!ff_cre_syncobj((BYTE)vol, &fs->sobj)) return FR_INT_ERR;
#endif
	}
	FatFs[vol] = fs;					/* Register new fs object 注册新的fs对象 */
	
	if (!fs||opt!=1){
		return FR_OK;	/* Do not mount now, it will be mounted later 现在不挂载，它将稍后挂载 */
	}
	res = find_volume(&fs,&path,0);	/* Force mounted the volume 强制挂载卷 */
	LEAVE_FF(fs,res);
}
...
FRESULT f_read (
	FIL* fp, 		/* Pointer to the file object 指向文件对象的指针*/
	void* buff,		/* Pointer to data buffer 数据缓冲区指针*/
	UINT btr,		/* Number of bytes to read 要读取的字节数*/
	UINT* br		/* Pointer to number of bytes read 指向已读字节数的指针*/
)
{
	FRESULT res;
...
FRESULT f_readdir (
	...
	FILINFO* fno		/* Pointer to file information to return 要返回的文件信息的指针*/
)
{
	FRESULT res;
	DEFINE_NAMEBUF;
