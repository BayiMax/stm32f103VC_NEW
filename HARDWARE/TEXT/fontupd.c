...
//字库存放起始地址 
#define FONTINFOADDR 	1024*1024*6 					//WarShip STM32F103 V3是从12M地址以后开始存放字库
														//前面12M被fatfs占用了.
...
u32 fupd_prog(u16 x,u16 y,u8 size,u32 fsize,u32 pos)
{
	...
	if(t!=prog)
	{
		sprintf(sprintf_buf,"o/o:%d",size);
		t=prog;
		if(t>100)t=100;
		LCDDebug(0,16*5,sprintf_buf);		//显示数值
	}
	return 0;					    
} 
//更新某一个
...
//返回值:0,更新成功;
//		 其他,错误代码.
u8 update_font(u16 x,u16 y,u8 size,char* src)
{	
	u8 *pname;
	u32 *buf;
	u8 res=0;		   
	...
	if(rval==0)//字库文件都存在.
	{
		LCDDebug(0,0,"Erasing sectors...");		//提示正在擦除扇区	
